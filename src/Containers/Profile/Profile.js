...
class Profile extends Reflux.Component {
  ...
  render() {
    ...
    const loading = this.state.loading || this.state.userStoreLoading;
    const { org, me } = this.state;

    return (
      <section className="profile">
        <Dimmer active={loading} page children={<Loader />} />
        {
          !org ? null :
          <Grid fluid>
            <Row>
            ...
            <Row>
              <Col xs={12} sm={4} className="flex-row flex-vhCenter" style={{padding: 10}}>
                <Image src={org.logo} responsive style={{maxHeight: 150}} />
              </Col>
              <Col xs={12} sm={8}>
                <ListGroup>
                  <ListGroupItem>
